...
# from unifmu.model import PythonModel
# import numpy as np

# def computeBalancesNew(inputs):
#     # Inputs desempaquetados (simulación sencilla)
...
#     balance = vfr_5 + vfr_8 - vfr_13

#     return balance

# class BalanceModel(PythonModel):
#     def __init__(self, instance_name: str, unifmu_context):
#         super().__init__(instance_name, unifmu_context)
#         self.inputs = np.zeros(18)
#         self.balance = 0.0

#     def do_step(self, current_time: float, step_size: float) -> bool:
#         self.balance = computeBalancesNew(self.inputs)
#         return True

#     def get_real(self, references: list[int]) -> list[float]:
#         # Siempre se espera que haya una sola salida
#         return [self.balance]

#     def set_real(self, references: list[int], values: list[float]) -> None:
#         # Reemplazamos las entradas por los nuevos valores
#         self.inputs = values


# # ========= SIMULAR FMU =========
# fmu_path = "BalanceFMU/BalanceFMU.fmu"
# input_values = np.array([[t] + [10.0]*18 for t in np.linspace(0, 10, 11)])  # dummy inputs
# input_dict = {"u{}".format(i): input_values[:, i+1] for i in range(18)}
# input_dict["time"] = input_values[:, 0]
# input_df = pd.DataFrame(input_dict)

# result = simulate_fmu(fmu_path, start_time=0, stop_time=10, input=input_df)

# # ========= COMPARAR =========
# manual_result = computeBalancesNew([10.0]*18)
# print("Resultado función directa:", manual_result)
# print("Resultado FMU:", result['balance'][-1])
